...
addBtn.addEventListener("click", () => {
    ...
    let url = `https://newnote.rf.gd/create.php?title=${ftitle.value}&desc=${fdesc.value}`;
    fetch(url).then((res) => {
...
function getData() {
    let url = `https://newnote.rf.gd/sonu.php?auth=ajdshuihdaskbjhiasd`;
    fetch(url).then((res) => {
        return res.text();
    }).then((data) => {
        // console.log(data);
...
function delNote(index = 9) {
    console.log(index);
    let data = index;
    let url = `https://newnote.rf.gd/del.php?auth=ajdshuihdaskbjhiasd&data=${data}`;
    itemBox.innerText = "";
